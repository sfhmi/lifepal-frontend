...
const ListProducts = (): React.JSX.Element => {
  ...
  const [sortMode, setSortMode] = useQueryState("srtm", { defaultValue: 'asc' })
  ...
  const { data: rawProducts, isLoading } = useSWR<{ products: Product[] }>(`${uri}${queryString.replaceAll(" ", "-")}`, API, {
    onSuccess: (response) => {
  ...
  const handleFilterProduct = useCallback((data: Product[]) => {
    ...
    <Fragment>
      <div className="grid grid-cols-2 items-center mb-4">
        ...
        <div className="flex flex-row gap-2 w-full">
          <Select classNames={{
            value: 'capitalize'
          }} variant="bordered" radius="sm" aria-label="sort-by" onChange={(e) => setSortBy(e.target.value)} defaultSelectedKeys={[sortBy]} className="max-w-xs" >
            {SORTBY.map((item: string) => (
              <SelectItem key={item} className="capitalize">{item}</SelectItem>
            ))}
          </Select>
          <Select classNames={{
            value: 'capitalize'
          }} variant="bordered" radius="sm" aria-label="sort-mode" onChange={(e) => setSortMode(e.target.value)} defaultSelectedKeys={[sortMode]} className="max-w-xs">
        ...
        {isLoading ? (
          ...
          <Fragment>
            {products?.map((item: Product, idx: number) => (
              <CardProduct clickHandle={handleCheckDetail} key={idx} item={item} />
            ))}
          </Fragment>
        )}
      </div>
      <ModalProduct isOpen={isOpen} onOpenChange={onOpenChange} data={selectedProduct} />
    </Fragment >
  )
}
